...
check_raster <- function(map, argument_name="") { #check existence of raster map
  ...
  # #add mapset name, unless already given. Otherwise, these checks may fall back on PERMANENT yielding true, but readRAST will fail later
  # if (!grepl(map, pattern = "@"))
  #   map = paste0(map,"@", cur_mapset) 
...
read_raster <- function(raster_name) { 
  ...
  if(!grepl(raster_name, pattern = "@")){
    ...
    #determine name of current mapset
    cur_mapset = execGRASS("g.mapset", flags="p", intern=TRUE)
    
    #case 2: raster in current mapset OR PERMANENT
    if(raster_name %in% unlist(attributes(execGRASS("g.list", parameters = list(type = "raster"))))){
    ...
    #case 3: raster in current mapset and PERMANENT > read from current mapset
    if(paste0(raster_name, "@", cur_mapset) %in% unlist(attributes(execGRASS("g.list", parameters = list(type = "raster"))))){
      
      tmp <- read_RAST(paste0(raster_name,"@", cur_mapset))
      tmp <- raster(tmp)
